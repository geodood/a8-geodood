...
public class JSpotBoard extends JPanel implements SpotBoard {
	...
	public JSpotBoard(int width, int height) {
		...
		for (int y=0; y<height; y++) {
			for (int x=0; x<width; x++) {
				...
				_spots[x][y] = new JSpot(bg, DEFAULT_BACKGROUND_LIGHT, DEFAULT_HIGHLIGHT_COLOR, this, x, y);
	...
	public JSpotBoard(int width, int height, Color background_color) {
		if (width < 1 || height < 1 || width > 500 || height > 500) {
			throw new IllegalArgumentException("Illegal spot board geometry");
		}
		setLayout(new GridLayout(height, width));
		...
		for (int y=0; y<height; y++) {
			for (int x=0; x<width; x++) {
				...
				_spots[x][y] = new JSpot(bg, bg, DEFAULT_HIGHLIGHT_COLOR, this, x, y);
				((JSpot)_spots[x][y]).setPreferredSize(preferred_size);
				add(((JSpot) _spots[x][y]));
			}			
		}
	...
	public JSpotBoard(int width, int height, String board_type) {
		
		if (!board_type.equals("columns")) {
			throw new IllegalArgumentException("Unspecified/Wrong board type");
		}
		...
		for (int y=0; y<height; y++) {
			for (int x=0; x<width; x++) {
				if (x % 2 == 0) {
					_spots[x][y] = new JSpot(DEFAULT_BACKGROUND_LIGHT, DEFAULT_BACKGROUND_LIGHT, DEFAULT_HIGHLIGHT_COLOR, this, x, y);
					((JSpot)_spots[x][y]).setPreferredSize(preferred_size);
					add(((JSpot) _spots[x][y]));
				} else {
